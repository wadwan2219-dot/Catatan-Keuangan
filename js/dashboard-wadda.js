 ...
 * Modul logika khusus untuk dashboard pribadi Wadda.
 * Bebas dikustomisasi khusus Wadda tanpa mempengaruhi pengguna lain.
 */

document.addEventListener('DOMContentLoaded', () => {
  const user = requireAuth();

  // Pastikan sesi aktif mengarah ke Wadda
  if (user.memberId !== 'wadda') {
    // Sesi diselaraskan ke Wadda jika masuk ke halaman ini
    user.memberId = 'wadda';
    user.uid = 'wadda';
  ...
  // Load Initial Dashboard Data for Wadda
...
function loadWaddaDashboardMetrics() {
  ...
  if (progressBar && progressPercentText) {
    ...
    if (percentage > 90) {
      ...
      progressBar.className = 'h-full bg-gradient-to-r from-indigo-500 to-purple-500 rounded-full transition-all duration-500';
      progressPercentText.className = 'text-xs font-bold text-indigo-400';
    }
  }

  // Render Recent Transactions (Top 5)
  renderWaddaRecentTransactions(transactions.slice(0, 5));
}

function renderWaddaRecentTransactions(recentList) {
  ...
  if (recentList.length === 0) {
    container.innerHTML = `
      <div class="text-center py-8 text-slate-400">
        ...
        <p class="text-sm font-medium">Belum ada transaksi pribadi Wadda</p>
      </div>
    `;
    return;
  }
